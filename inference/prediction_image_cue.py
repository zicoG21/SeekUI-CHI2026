...
def parse_args():
    parser = argparse.ArgumentParser(description="Scanpath prediction with a target crop image cue.")
...
def main():
    ...
    for idx in tqdm(range(len(examples)), total=len(examples)):
        ...
        prompt = (
            f"Given the first image as a GUI screenshot with width {width} and height {height}, "
            "and the second image as a visual crop of the target UI element, "
            "predict the human-like scanpath for finding that target in the GUI.\n"
        ...
        thinking, answer, points = parse_points(content)
        result = dict(example)
        ...
        if len(points) == 0:
            result["prediction"] = [[width // 2, height // 2]]
            result["prediction_fallback"] = "image_center"
            print(f"Warning: no points parsed for index {idx}, sample {sample_id}")
