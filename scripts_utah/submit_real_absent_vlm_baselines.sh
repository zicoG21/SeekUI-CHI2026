...
for variant in "${VARIANTS[@]}"; do
  ...
  output="$(
    ...
    VLM_LIMIT="$VLM_LIMIT" \
      sbatch "${SBATCH_ARGS[@]}" scripts_utah/vlm_presence_baseline.slurm
  )"
  echo "$output"
...
if [[ "${RUN_EVIDENCE:-0}" != "0" ]]; then
  ...
  for variant in "${EVIDENCE_VARIANTS[@]}"; do
    ...
    output="$(
      ...
      VLM_LIMIT="$VLM_LIMIT" \
        sbatch "${SBATCH_ARGS[@]}" scripts_utah/vlm_evidence_presence.slurm
    )"
    echo "$output"
    JOB_IDS+=("$(awk '/Submitted batch job/ {job=$4} END {print job}' <<<"$output")")
  done
...
if [[ "${SUBMIT_SUMMARY:-1}" != "0" && "${#JOB_IDS[@]}" -gt 0 ]]; then
  ...
  sbatch "${SUMMARY_ARGS[@]}" scripts_utah/summarize_real_absent_results.slurm
fi
