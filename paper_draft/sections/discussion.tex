...
\paragraph{Implications for GUI evaluation.}
Forced-choice synthetic users can overestimate findability: if a model must always return an element, it may make an interface look searchable even when the desired action is absent from the screen. Uncertainty-aware search changes the evaluation question. A GUI can fail because the target is not exposed, because the screen contains a strong distractor, or because the target requires navigation or contextual state. A target-absence verifier can therefore support UI evaluation by flagging screens where the expected action is missing or where visible distractors attract unsupported commitments.
